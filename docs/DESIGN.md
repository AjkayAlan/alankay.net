...
- **Base pairs**: the rungs cycle through P·P·B·V. Currently **10 rungs** =
...
The favicon set (`favicon.svg`, `favicon.ico`, `apple-touch-icon.png`) is a
**static DNA helix**: two backbone strands with four colored rungs in the
identity sequence (P·P·B·V). It's generated by `scripts/generate-assets.py`
alongside the portrait. The helix is the site's signature, so the favicon
echoes it at a glance.
