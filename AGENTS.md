...
**DNA → four influences → four letters (H·B·C·P) with fixed colors.**
...
2. **The four influence colors are sacred.** Hockey `#2d9cdb`, Band `#d9a521`,
   Computers `#23a26d`, Parents `#e0675a`. They never change and never differ
...
4. **Letter + color + anchor mapping is a unit.** If you change an influence's
   letter or color, update `src/data/influences.ts`, `DnaHelix.astro`, the chips,
   and the About badges together.
...
8. **Don't rewrite the About copy.** It's near-verbatim from the owner; fix
   typos, don't re-author.
...
- **Too few rungs** → the sequence is `RUNGS` (10 = 2.5× the H·B·C·P repeat).
